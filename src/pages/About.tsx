  ...
  return (
    <Layout>
              ...
              {lang === "he" && (
                <div className="mt-8 pt-8 border-t border-border">
                  <p className="font-bold text-foreground leading-snug" style={{ fontSize: "clamp(1.1rem, 2vw, 1.4rem)" }}>
                    {"בעידן ה-AI רק עורך דין שמומחה בבינה מלאכותית".split("שמומחה").map((part, i, arr) =>
                      i < arr.length - 1 ? (
                        <span key={i}>
                          {part}
      ...
      <section className="relative h-56 md:h-80 overflow-hidden">
        <img src={officeImg} alt={lang === "he" ? "משרד HY Law Offices, גבעתיים" : "HY Law Offices, Givatayim"} className="w-full h-full object-cover object-[50%_65%]" loading="lazy" />
